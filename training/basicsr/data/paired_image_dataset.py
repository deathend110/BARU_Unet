...
class Dataset_PairedImage(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        elif self.opt.get('pair_by_index', False):
            # ponytail: 两目录同名文件按 sorted 顺序 1:1 配对，跳过 filename assert
            lq_names = sorted(os.listdir(self.lq_folder))
            gt_names = sorted(os.listdir(self.gt_folder))
            assert len(lq_names) == len(gt_names), \
                f'LQ({len(lq_names)}) 和 GT({len(gt_names)}) 图片数量不一致'
            self.paths = [{'lq_path': os.path.join(self.lq_folder, lq),
                           'gt_path': os.path.join(self.gt_folder, gt)}
                          for lq, gt in zip(lq_names, gt_names)]
